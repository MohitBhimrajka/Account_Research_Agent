...
export function AboutYouStep() {
 ...
 useEffect(() => {
  ...
  // --- Render ---
  return (
    <div className="space-y-6"> {/* Removed animation class */}
      ...
      <div>
          ...
          <div>
            <label htmlFor="userCompany" className="block text-sm font-medium text-muted-foreground mb-1 sr-only">
              Your Company Name
            </label>
            <input
              id="userCompany"
            ...
            />
            {errors.userCompany && (
              <p id="userCompanyError" className="text-destructive text-xs mt-1 px-1">{errors.userCompany.message}</p>
            )}
          </div>
      </div>
      ...
      <div className="mt-8 pt-6 border-t border-border/50">
        <h3 className="text-lg font-semibold text-card-foreground mb-4">Report Customization</h3>
        ...
        {!isLoadingLanguages && !isLoadingSections && !languagesError && !sectionsError && availableLanguages && availableSections && (
           <div className="space-y-6">
              {/* Language Selection */}
              <div>
                <label htmlFor="language" className="block text-sm font-medium text-muted-foreground mb-2 flex items-center gap-1">
                ...
                <div className="relative">
                    ...
                    >
                      ...
                      {Object.entries(availableLanguages).map(([key, value]) => (
                        <option key={key} value={value} className="bg-background text-foreground">{value}</option>
                      ))}
                    </select>
                    <ChevronsUpDown className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground pointer-events-none" />
                </div>
                {errors.language && (
                  <p id="languageError" className="text-destructive text-xs mt-1 px-1">{typeof errors.language.message === 'string' ? errors.language.message : 'Please select a language.'}</p>
                )}
              </div>

              {/* Sections Selection */}
              <Accordion type="single" collapsible className="w-full border border-input rounded-md overflow-hidden bg-background">
                <AccordionItem value="sections" className="border-b-0">
                  <AccordionTrigger className="px-4 py-3 text-card-foreground hover:bg-secondary/30 hover:no-underline font-medium text-base data-[state=open]:bg-secondary/30 data-[state=open]:border-b border-input flex items-center gap-1 transition-colors">
                  ...
                  <AccordionContent className="p-4 border-t border-border">
                    <div className="space-y-4">
                      <div className="flex items-center justify-between mb-3">
                        <p className="text-muted-foreground text-sm">Defaults to all sections if none are selected.</p>
                        <Button
                        ...
                        {Array.isArray(availableSections) && availableSections.map((section) => (
                          <div key={section.id} className="flex items-center space-x-2 group">
                            ...
                            <label
                              ...
                              className="text-sm font-medium leading-none text-foreground group-hover:text-accent peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer select-none transition-colors"
                            >
                              {section.title}
                            </label>
                          </div>
                      ...
                      {/* Display general form error for sections if needed, though less likely for array validation */}
                       {errors.sections && typeof errors.sections.message === 'string' && (
                         <p className="text-destructive text-xs mt-1 px-1">{errors.sections.message}</p>
                       )}
                    </div>
                  </AccordionContent>
                </AccordionItem>
