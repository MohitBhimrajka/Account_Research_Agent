...
def get_user_input() -> tuple[str, str, list[str], list[tuple[str, str]]]:
    ...
    while True:
        languages = input(
            "\nSelect language(s) (1-10, comma separated, default is 1 for Japanese): "
        ).strip()
        if not languages:
            languages = "1"
        ...
        if all(key in AVAILABLE_LANGUAGES for key in language_keys):
            break
        print(
            "Invalid selection. Please choose numbers between 1 and 10, separated by commas."
        )
    ...
    while True:
        sections = input(
            "\nSelect sections (comma-separated numbers, or 0 for all): "
        ).strip()
        if not sections:
            sections = "0"
        ...
        try:
            ...
            if 0 in section_indices:
                ...
                selected_prompts = [
                    PROMPT_FUNCTIONS[idx - 1] for idx in section_indices
                ]
                break
            else:
                print(
                    f"Invalid selection. Please choose numbers between 0 and {len(PROMPT_FUNCTIONS)}."
                )
        except ValueError:
...
def run_generation(
    ...
    language: str,
    selected_prompts: list[tuple[str, str]],
    progress=None,
    language_task_id=None,
):
    ...
    config = {
        ...
        "sections": [
            section[0] for section in selected_prompts
    ...
    if not isinstance(progress, DummyProgress):
        for prompt_name, _ in selected_prompts:
            ...
            section_tasks[prompt_name] = progress.add_task(
                task_desc, total=1, visible=True
            )
    ...
    with ThreadPoolExecutor(max_workers=max_workers_prompts) as executor:
        ...
        for prompt_name, future in futures:
            try:
                ...
                results[prompt_name] = {"status": "error", "error": str(e)}
                if prompt_name in section_tasks:
                    progress.update(
                        section_tasks[prompt_name],
    ...
    token_stats = {
        ...
        "summary": {
            ...
            "failed_prompts": sum(
                ...
                if r.get("status") in ["error", "interrupted"]
            ),
