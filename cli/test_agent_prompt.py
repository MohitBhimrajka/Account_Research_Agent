...
def main():
    try:
        ...
        company_name, platform_company_name, language_keys, selected_prompts = (
            get_user_input()
        )
        ...
        for language_key in language_keys:
            ...
            console.print(
                f"Using model: {LLM_MODEL} with temperature: {LLM_TEMPERATURE}"
            )
        ...
        with Progress(
            ...
            language_tasks = {
                lang: progress.add_task(
                    f"[cyan]{lang} Progress", total=len(selected_prompts)
                )
        ...
        if results:
            ...
            total_execution_time = sum(
                stats["summary"]["total_execution_time"] for _, stats, _ in results
            )
            total_tokens = sum(
                stats["summary"]["total_tokens"] for _, stats, _ in results
            )
            total_successful = sum(
                stats["summary"]["successful_prompts"] for _, stats, _ in results
            )
            total_failed = sum(
                stats["summary"]["failed_prompts"] for _, stats, _ in results
            )
