    ...
    for company_dir in output_dir.glob("*_*_*"):
        ...
        try:
            ...
            if len(parts) >= 2:
                ...
                if (
                    markdown_dir.exists()
                    and markdown_dir.is_dir()
                    and list(markdown_dir.glob("*.md"))
                ):
            ...
            console.print(
                f"[yellow]Warning: Could not process {company_dir}: {e}[/yellow]"
            )
            continue
...
def find_company_output_dir(
    output_dir: Path, company: str, language: str
) -> Optional[Path]:
...
def display_companies(companies: Dict[str, Set[str]]) -> None:
    ...
    for idx, (company, languages) in enumerate(companies.items(), 1):
        table.add_row(str(idx), company, ", ".join(sorted(languages)))
...
def copy_company_files_from_output(
    input_path: Path, company: str, language: str, target_dir: Path
) -> bool:
...
def copy_company_files(
    ...
    for pattern in patterns:
        for source_file in input_path.glob(f"**/{pattern}"):
            ...
            shutil.copy2(source_file, target_file)
            console.print(
                f"[green]Copied {source_file.name} → {target_file.name}[/green]"
            )
            found_files = True
...
def main():
    ...
    parser.add_argument(
        "--company-name", "-c", type=str, help="Company name for the report"
    )
    parser.add_argument("--language", "-l", type=str, help="Language for the report")
    parser.add_argument(
        "--template", "-t", type=str, help="Custom template path (optional)"
    )
    parser.add_argument(
        "--output-dir", "-o", type=str, help="Output directory for the PDF"
    )
    parser.add_argument(
    ...
    if args.interactive or (not args.company_name and not args.language):
        ...
        if not company_name or not language:
            console.print(
                "[red]Error: Both company name and language are required in non-interactive mode.[/red]"
            )
            sys.exit(1)
    ...
    base_dir = (
        ...
        if args.output_dir
        else Path(f"pdf_output/{company_name}_{language}_{timestamp}")
    )
    ...
    if not copy_company_files(input_path, company_name, language, markdown_dir):
        console.print(
            f"[red]Error: No matching files found for {company_name} in {language}[/red]"
        )
        sys.exit(1)
